...
/** Helper function to hash anything that can be referenced as bytes. */
...
fn read_recipes_from_fs(app_handle: &tauri::AppHandle) -> Result<Vec<Recipe>, String> {
	...
	for entry in fs::read_dir(recipes_dir).map_err(|e| e.to_string())? {
		...
		if path.extension().and_then(|s| s.to_str()) == Some("md") {
			...
			// Create hashes of the contents & path to facilitate change detection
			let content_hash = hash(&content);
			let path_hash = hash(path.as_os_str().as_encoded_bytes());
...
/**
 * Read recipes from the filesystem and write them to Tauri state.
 * Currently this is naive; it always reads and writes all recipes.
 */
#[tauri::command]
pub async fn read_recipe_data(
	...
	let recipes = read_recipes_from_fs(&app_handle)?;
